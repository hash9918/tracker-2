...
import { Calendar, Play, RotateCcw, Clock, Smile, Award, CheckCircle2, ChevronLeft, ChevronRight, XCircle } from 'lucide-react';
...
const HistoryReplay = () => {
  ...
  const handleLastTuesday = () => {
    const d = new Date();
    const day = d.getDay();
    // Calculate difference to last Tuesday
    const diff = d.getDate() - day + (day >= 2 ? 2 : -5);
    const lastTuesday = new Date(d.setDate(diff));
    setSelectedDate(formatDateLocal(lastTuesday));
  };
  ...
  const startReplay = () => {
    ...
    completedBlocks.forEach((block, idx) => {
      ...
      }, (idx + 1) * 800); // 800ms between each checkoff
    });
  };
                    ...
                    >
                      ...
                      <div>
                        {isChecked ? (
                          ...
                          /* If replaying, show a greyed dot until it triggers */
